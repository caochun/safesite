    ...
    if model_path.suffix.lower() == ".onnx":
        ...
        def detect(frame: np.ndarray) -> List[Detection]:
            ...
            for det in outputs:
                ...
                left = int((x_center - w / 2) * frame_w)
                top = int((y_center - h / 2) * frame_h)
                width_px = int(w * frame_w)
                height_px = int(h * frame_h)
    ...
    def detect(frame: np.ndarray) -> List[Detection]:
        ...
        if classes is not None:
            for class_id, confidence, box in zip(classes.flatten(), scores.flatten(), boxes):
                ...
                if args.class_id is not None and class_id != args.class_id:
                    continue
                detections.append((tuple(int(v) for v in box), class_id, confidence))
        return detections
...
class RecordingSink:
    def __init__(self, width: int, height: int, framerate: int, bitrate_kbps: int, output_path: Path):
        ...
        appsrc = self.pipeline.get_by_name("recsrc")
        if not isinstance(appsrc, GstApp.AppSrc):
            raise RuntimeError("Failed to get appsrc from recording pipeline.")
        self.appsrc = appsrc

        filesink = self.pipeline.get_by_name("recfilesink")
...
def main() -> int:
    ...
    window_name = "GStreamer Object Detection"
    try:
        while True:
            ...
            for (x, y, w, h), class_id, confidence in detections:
                ...
                if args.class_id is None or class_id == args.class_id:
                    triggered = True
